...
st.write('''# Portfolio Tracker ''')  # title
st.sidebar.image("Images/bulls-bears.jpg", width=250,
...
# Stock Price Prediction Section Ends Here
    ...
    st.markdown('<p class="big-font">Portfolio Tracker is a web application that allows users to visualize Stock Performance Comparison, Real-Time Stock Prices and Stock Price Prediction. This application is developed using Streamlit. Streamlit is an open source app framework in Python language. It helps users to create web apps for Data Science and Machine Learning in a short time. This Project is developed by Kushagra Rai , Ranveer Singh and Som Shekhar Singh.<br> Cheers!</p>', unsafe_allow_html=True)
